...
#    fi
    ...
    if [ -z "$OAT2DEXJAR" ]; then
        export OAT2DEXJAR="$CM_ROOT"/vendor/cm/build/tools/smali/oat2dex.jar
    fi
    ...
    for ARCH in $ARCHES; do
        local BOOTOAT="$TMPDIR/boot_$ARCH.oat"
        local BOOTDEX="$TMPDIR/dex"

        if [ ! -d "$BOOTDEX" ]; then
            java -jar "$OAT2DEXJAR" boot "$BOOTOAT"
        fi

        local TARGET_BASENAME=$(basename "$OEM_TARGET" ."${OEM_TARGET##*.}")
        # local OAT="$(dirname "$OEM_TARGET")/oat/$ARCH/${TARGET_BASENAME}.odex"
        local OAT="$(dirname "$OEM_TARGET")/$ARCH/${TARGET_BASENAME}.odex"

        if get_file "$OAT" "$TMPDIR" "$SRC"; then
            echo "Deodexing: $OAT"
            # java -jar "$BAKSMALIJAR" $BAKSMALI_API_LEVEL -x -o "$TMPDIR/dexout" -c "$BOOTOAT" -d "$TMPDIR" "$TMPDIR/$(basename "$OAT")"
        # elif [[ "$CM_TARGET" =~ .jar$ ]]; then
            # try to extract classes.dex from boot.oat for framework jars
            # java -jar "$BAKSMALIJAR" $BAKSMALI_API_LEVEL -x -o "$TMPDIR/dexout" -c "$BOOTOAT" -d "$TMPDIR" -e "/$OEM_TARGET" "$BOOTOAT"
        elif [[ "$CM_TARGET" =~ .jar$ ]] && get_file "system/framework/$ARCH/${TARGET_BASENAME}.odex" "$TMPDIR/${TARGET_BASENAME}.odex" "$SRC"; then
            echo "Deodexing jar: $CM_TARGET"
        else
            continue
        fi

        java -jar "$OAT2DEXJAR" "$TMPDIR/${TARGET_BASENAME}.odex" "$BOOTDEX"

        # java -jar "$SMALIJAR" "$TMPDIR/dexout" -o "$TMPDIR/classes.dex" && break
        mv "$TMPDIR/${TARGET_BASENAME}.dex" "$TMPDIR/classes.dex" && break
    done
